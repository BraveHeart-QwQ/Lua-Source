...
*/
static int
tonumeral(const expdesc* e, TValue* v)
{
...
luaK_concat(FuncState* fs, int* l1, int l2)
{
    ...
    else if (*l1 == NO_JUMP) /* no original list? */ *l1 = l2; /* 'l1' points to 'l2' */
    else {
        int list = *l1;
        int next;
...
*/
void
luaK_patchclose(FuncState* fs, int list, int level)
{
...
*/
static int
codeextraarg(FuncState* fs, int a)
{
...
*/
int
luaK_codek(FuncState* fs, int reg, int k)
{
    if (k <= MAXARG_Bx) return luaK_codeABx(fs, OP_LOADK, reg, k);
    else {
...
*/
static void
freeexps(FuncState* fs, expdesc* e1, expdesc* e2)
{
...
static int
code_loadbool(FuncState* fs, int A, int b, int jump)
{
...
*/
static void
negatecondition(FuncState* fs, expdesc* e)
{
...
*/
static int
validop(int op, TValue* v1, TValue* v2)
{
    switch (op) {
    case LUA_OPBAND:
    case LUA_OPBOR:
...
*/
static int
constfolding(FuncState* fs, int op, expdesc* e1, const expdesc* e2)
{
    TValue v1, v2, res;
    ...
    else { /* folds neither NaN nor 0.0 (to avoid problems with -0.0) */ lua_Number n = fltvalue(&res);
...
*/
static void
codeunexpval(FuncState* fs, OpCode op, expdesc* e, int line)
{
...
*/
static void
codebinexpval(FuncState* fs, OpCode op, expdesc* e1, expdesc* e2, int line)
{
...
*/
static void
codecomp(FuncState* fs, BinOpr opr, expdesc* e1, expdesc* e2)
{
...
*/
void
luaK_prefix(FuncState* fs, UnOpr op, expdesc* e, int line)
{
...
*/
void
luaK_posfix(FuncState* fs, BinOpr op, expdesc* e1, expdesc* e2, int line)
{
    switch (op) {
    case OPR_AND: {
...
luaK_setlist(FuncState* fs, int base, int nelems, int tostore)
{
    ...
    else luaX_syntaxerror(fs->ls, "constructor too long");

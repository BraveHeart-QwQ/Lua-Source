...
/* semantic error */
static l_noret
semerror(LexState* ls, const char* msg)
{
...
static void
checklimit(FuncState* fs, int v, int l, const char* what)
{
...
static void
new_localvar(LexState* ls, TString* name)
{
    FuncState* fs = ls->fs;
    Dyndata* dyd  = ls->dyd;
    ...
    luaM_growvector(
        ls->L, dyd->actvar.arr, dyd->actvar.n + 1, dyd->actvar.size, Vardesc, MAX_INT, "local variables"
    );
...
static void
new_localvarliteral_(LexState* ls, const char* name, size_t sz)
{
...
static void
removevars(FuncState* fs, int tolevel)
{
...
static int
searchupvalue(FuncState* fs, TString* name)
{
    int i;
...
static int
newupvalue(FuncState* fs, TString* name, expdesc* v)
{
    Proto* f    = fs->f;
...
static void
closegoto(LexState* ls, int g, Labeldesc* label)
{
    int i;
    FuncState* fs = ls->fs;
    ...
    if (gt->nactvar < label->nactvar) {
        ...
        const char* msg = luaO_pushfstring(
            ls->L, "<goto %s> at line %d jumps into the scope of local '%s'", getstr(gt->name), gt->line,
            getstr(vname)
        );
        semerror(ls, msg);
    }
...
*/
static void
findgotos(LexState* ls, Labeldesc* lb)
{
    Labellist* gl = &ls->dyd->gt;
...
*/
static void
movegotosout(FuncState* fs, BlockCnt* bl)
{
    int i         = bl->firstgoto;
...
static void
enterblock(FuncState* fs, BlockCnt* bl, lu_byte isloop)
{
    bl->isloop     = isloop;
...
*/
static l_noret
undefgoto(LexState* ls, Labeldesc* gt)
{
    const char* msg =
...
*/
static void
codeclosure(LexState* ls, expdesc* v)
{
...
static void
open_func(LexState* ls, FuncState* fs, BlockCnt* bl)
{
    Proto* f;
...
*/
static int
block_follow(LexState* ls, int withuntil)
{
    switch (ls->t.token) {
    case TK_ELSE:
    case TK_ELSEIF:
...
static void
fieldsel(LexState* ls, expdesc* v)
{
    /* fieldsel -> ['.' | ':'] NAME */
    FuncState* fs = ls->fs;
    expdesc key;
...
static void
field(LexState* ls, struct ConsControl* cc)
{
...
static void
body(LexState* ls, expdesc* e, int ismethod, int line)
{
...
static void
funcargs(LexState* ls, expdesc* f, int line)
{
    FuncState* fs = ls->fs;
    expdesc args;
    int base, nparams;
...
static void
suffixedexp(LexState* ls, expdesc* v)
{
    /* suffixedexp ->
...
assignment(LexState* ls, struct LHS_assign* lh, int nvars)
{
    ...
    else { /* assignment -> '=' explist */ int nexps;
        checknext(ls, '=');
        nexps = explist(ls, &e);
...
static void
gotostat(LexState* ls, int pc)
{
    int line = ls->linenumber;
    TString* label;
    int g;
    if (testnext(ls, TK_GOTO)) label = str_checkname(ls);
    else {
...
static void
checkrepeated(FuncState* fs, Labellist* ll, TString* label)
{
    int i;
    for (i = fs->bl->firstlabel; i < ll->n; i++) {
        if (eqstr(label, ll->arr[i].name)) {
            const char* msg = luaO_pushfstring(
                fs->ls->L, "label '%s' already defined on line %d", getstr(label), ll->arr[i].line
            );
            semerror(fs->ls, msg);
        }
    }
...
static void
labelstat(LexState* ls, TString* label, int line)
{
    /* label -> '::' NAME '::' */
    FuncState* fs = ls->fs;
...
forbody(LexState* ls, int base, int line, int nvars, int isnum)
{
    ...
    else { /* generic for */ luaK_codeABC(fs, OP_TFORCALL, base, 0, nvars);
        luaK_fixline(fs, line);
...
fornum(LexState* ls, TString* varname, int line)
{
    ...
    else { /* default step = 1 */ luaK_codek(fs, fs->freereg, luaK_intK(fs, 1));
        luaK_reserveregs(fs, 1);
    }
...
static void
test_then_block(LexState* ls, int* escapelist)
{
    ...
    if (ls->t.token == TK_GOTO || ls->t.token == TK_BREAK) {
        ...
        else /* must skip over 'then' part if condition is false */ jf = luaK_jump(fs);
    }
...
exprstat(LexState* ls)
{
    ...
    else { /* stat -> func */ check_condition(ls, v.v.k == VCALL, "syntax error");
...
retstat(LexState* ls)
{
    ...
    if (block_follow(ls, 1) || ls->t.token == ';') first = nret = 0; /* return no values */
    else {
...
*/
static void
mainfunc(LexState* ls, FuncState* fs)
{
    BlockCnt bl;
    expdesc v;
    open_func(ls, fs, &bl);

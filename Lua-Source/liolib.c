...
{
    return (
        *mode != '\0' && strchr("rwa", *(mode++)) != NULL && (*mode != '+' || (++mode, 1)) && /* skip if char
                                                                                                 is '+' */
...
io_type(lua_State* L)
{
    ...
    if (p == NULL) lua_pushnil(L); /* not a file */
    else if (isclosed(p)) lua_pushliteral(L, "closed file");
    else lua_pushliteral(L, "file");
    return 1;
}
...
f_tostring(lua_State* L)
{
    ...
    if (isclosed(p)) lua_pushliteral(L, "file (closed)");
    else lua_pushfstring(L, "file (%p)", p->f);
    return 1;
}
...
io_lines(lua_State* L)
{
    ...
    else { /* open a new file */ const char* filename = luaL_checkstring(L, 1);
...
read_number(lua_State* L, FILE* f)
{
    ...
    if (lua_stringtonumber(L, rn.buff))         /* is this a valid number? */
        return 1;                               /* ok */
    else { /* invalid format */ lua_pushnil(L); /* "result" to be removed */
        return 0;                               /* read fails */
    }
}
...
f_seek(lua_State* L)
{
    ...
    if (op) return luaL_fileresult(L, 0, NULL); /* error */
    else {

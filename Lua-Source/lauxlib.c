...
pushfuncname(lua_State* L, lua_Debug* ar)
{
    ...
    else if (*ar->what == 'm') /* main? */ lua_pushliteral(L, "main chunk");
...
static int
typeerror(lua_State* L, int arg, const char* tname)
{
    const char* msg;
    ...
    else typearg = luaL_typename(L, arg); /* standard name */
...
luaL_checkstack(lua_State* L, int space, const char* msg)
{
    if (!lua_checkstack(L, space)) {
        if (msg) luaL_error(L, "stack overflow (%s)", msg);
        else luaL_error(L, "stack overflow");
    }
}
...
{
    if (lua_isnumber(L, arg)) luaL_argerror(L, arg, "number has no integer representation");
...
static void*
newbox(lua_State* L, size_t newsize)
{
...
luaL_prepbuffsize(luaL_Buffer* B, size_t sz)
{
    ...
    if (B->size - B->n < sz) { /* not enough space? */
        ...
        if (buffonstack(B)) newbuff = (char*)resizebox(L, -1, newsize);
        else { /* no buffer yet */ newbuff = (char*)newbox(L, newsize);
...
luaL_ref(lua_State* L, int t)
{
    ...
    else /* no free elements */ ref = (int)lua_rawlen(L, t) + 1; /* get a new reference */
    lua_rawseti(L, t, ref);
    return ref;
}
...
luaL_tolstring(lua_State* L, int idx, size_t* len)
{
    if (luaL_callmeta(L, idx, "__tostring")) { /* metafield? */
        ...
        case LUA_TNUMBER: {
            if (lua_isinteger(L, idx)) lua_pushfstring(L, "%I", (LUAI_UACINT)lua_tointeger(L, idx));
            else lua_pushfstring(L, "%f", (LUAI_UACNUMBER)lua_tonumber(L, idx));
            break;
        }
        case LUA_TSTRING:
            lua_pushvalue(L, idx);
...
{
    if (lua_getfield(L, idx, fname) == LUA_TTABLE) return 1; /* table already there */
    else {
        lua_pop(L, 1); /* remove previous result */
...
l_alloc(void* ud, void* ptr, size_t osize, size_t nsize)
{
    ...
    else { /* cannot fail when shrinking a block */ void* newptr = realloc(ptr, nsize);
        if (newptr == NULL && ptr != NULL && nsize <= osize) return ptr; /* keep the original block */
        else /* no fail or not shrinking */ return newptr;               /* use the new block */
    }
}
...
luaL_checkversion_(lua_State* L, lua_Number ver, size_t sz)
{
    ...
    if (v != lua_version(NULL)) luaL_error(L, "multiple Lua VMs detected");
        ...
        luaL_error(
            L, "version mismatch: app. needs %f, Lua core provides %f", (LUAI_UACNUMBER)ver, (LUAI_UACNUMBER)*v
        );
}

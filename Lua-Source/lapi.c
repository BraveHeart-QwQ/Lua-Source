...
static TValue*
index2addr(lua_State* L, int idx)
{
    CallInfo* ci = L->ci;
    if (idx > 0) {
    ...
    else if (idx == LUA_REGISTRYINDEX) return &G(L)->l_registry;
    else { /* upvalues */ idx = LUA_REGISTRYINDEX - idx;
...
lua_checkstack(lua_State* L, int n)
{
    ...
    else { /* no; need to grow stack */ int inuse = cast_int(L->top - L->stack) + EXTRA_STACK;
        if (inuse > LUAI_MAXSTACK - n) /* can grow without overflow? */
            res = 0;                   /* no */
        else /* try to grow stack */ res = (luaD_rawrunprotected(L, &growstack, &n) == LUA_OK);
    }
...
lua_arith(lua_State* L, int op)
{
    ...
    else { /* for unary operations, add fake 2nd operand */ api_checknelems(L, 1);
...
lua_tocfunction(lua_State* L, int idx)
{
    ...
    else if (ttisCclosure(o)) return clCvalue(o)->f;
...
auxsetstr(lua_State* L, const TValue* t, const char* k)
{
    ...
    TString* str = luaS_new(L, k);
    api_checknelems(L, 1);
    if (luaV_fastset(L, t, str, slot, luaH_getstr, L->top - 1)) L->top--; /* pop value */
    else {
...
lua_seti(lua_State* L, int idx, lua_Integer n)
{
    ...
    api_checknelems(L, 1);
    t = index2addr(L, idx);
    if (luaV_fastset(L, t, n, slot, luaH_getint, L->top - 1)) L->top--; /* pop value */
    else {
        setivalue(L->top, n);
        api_incr_top(L);
        luaV_finishset(L, t, L->top - 1, L->top - 2, slot);
...
#define checkresults(L, na, nr)                                         \
    api_check(                                                          \
        L, (nr) == LUA_MULTRET || (L->ci->top - L->top >= (nr) - (na)), \
...
lua_callk(lua_State* L, int nargs, int nresults, lua_KContext ctx, lua_KFunction k)
{
    ...
    else /* no continuation or no yieldable */ luaD_callnoyield(L, func, nresults); /* just do the call */
...
lua_pcallk(lua_State* L, int nargs, int nresults, int errfunc, lua_KContext ctx, lua_KFunction k)
{
    ...
    else { /* prepare continuation (call is already protected by 'resume') */ CallInfo* ci = L->ci;
...
lua_dump(lua_State* L, lua_Writer writer, void* data, int strip)
{
    ...
    api_checknelems(L, 1);
    o = L->top - 1;
    if (isLfunction(o)) status = luaU_dump(L, getproto(o), writer, data, strip);
    else status = 1;
    lua_unlock(L);
    return status;
}
...
lua_gc(lua_State* L, int what, int data)
{
    ...
    case LUA_GCSTEP: {
        ...
        else { /* add 'data' to total debt */ debt = cast(l_mem, data) * 1024 + g->GCdebt;
            luaE_setdebt(g, debt);
            luaC_checkGC(L);
        }
...
lua_next(lua_State* L, int idx)
{
    ...
    else /* no more elements */ L->top -= 1; /* remove key */
    lua_unlock(L);
    return more;
}
...
lua_concat(lua_State* L, int n)
{
    ...
    else if (n == 0) { /* push empty string */ setsvalue2s(L, L->top, luaS_newlstr(L, "", 0));
        api_incr_top(L);
    }
...
static const char*
aux_upvalue(StkId fi, int n, TValue** val, CClosure** owner, UpVal** uv)
{
    switch (ttype(fi)) {
...
static UpVal**
getupvalref(lua_State* L, int fidx, int n)
{
    LClosure* f;

...
luaD_throw(lua_State* L, int errcode)
{
    ...
    else { /* thread has no error handler */ global_State* g = G(L);
...
luaD_shrinkstack(lua_State* L)
{
    ...
    else luaE_shrinkCI(L);                                  /* shrink list */
    /* if thread is currently not handling a stack overflow and its
       good size is smaller than current size, shrink its stack */
    if (inuse <= (LUAI_MAXSTACK - EXTRA_STACK) && goodsize < L->stacksize) luaD_reallocstack(L, goodsize);
    else /* don't change stack */ condmovestack(L, {}, {}); /* (change only for debugging) */
}


void
luaD_inctop(lua_State* L)
{
    luaD_checkstack(L, 1);
    L->top++;
}
...
*/
void
luaD_hook(lua_State* L, int event, int line)
{
...
static void
callhook(lua_State* L, CallInfo* ci)
{
...
*/
static int
moveresults(lua_State* L, const TValue* firstResult, StkId res, int nres, int wanted)
{
...
*/
int
luaD_poscall(lua_State* L, CallInfo* ci, StkId firstResult, int nres)
{
    StkId res;
...
luaD_precall(lua_State* L, StkId func, int nresults)
{
    ...
    case LUA_TLCL: { /* Lua function: prepare its call */
        ...
        if (p->is_vararg) base = adjust_varargs(L, p, n);
...
*/
static void
finishCcall(lua_State* L, int status)
{
    CallInfo* ci = L->ci;
    int n;
...
unroll(lua_State* L, void* ud)
{
    ...
    while (L->ci != &L->base_ci) {                  /* something in the stack */
        ...
        else { /* Lua function */ luaV_finishOp(L); /* finish interrupted instruction */
...
resume(lua_State* L, void* ud)
{
    ...
    else { /* resuming from previous yield */ lua_assert(L->status == LUA_YIELD);
...
lua_yieldk(lua_State* L, int nresults, lua_KContext ctx, lua_KFunction k)
{
    ...
    if (L->nny > 0) {
        if (L != G(L)->mainthread) luaG_runerror(L, "attempt to yield across a C-call boundary");

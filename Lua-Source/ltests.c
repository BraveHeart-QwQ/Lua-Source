...
static void
pushobject(lua_State* L, const TValue* o)
{
    setobj2s(L, L->top, o);
    api_incr_top(L);
}
...
static void
freeblock(Memcontrol* mc, Header* block)
{
    if (block) {
...
void*
debug_realloc(void* ud, void* b, size_t oldsize, size_t size)
{
    ...
    if (size == 0) {
        ...
        for (i = 0; i < MARKSIZE; i++) *(cast(char*, newblock + 1) + size + i) = MARK;
...
static int
testobjref1(global_State* g, GCObject* f, GCObject* t)
{
    if (isdead(g, t)) return 0;
    if (!issweepphase(g)) return !(isblack(f) && iswhite(t));
    else return 1;
}


static void
printobj(global_State* g, GCObject* o)
{
    printf(
        "||%s(%p)-%c(%02X)||", ttypename(novariant(o->tt)), (void*)o,
        isdead(g, o) ? 'd'
        : isblack(o) ? 'b'
        : iswhite(o)
            ? 'w'
...
static int
testobjref(global_State* g, GCObject* f, GCObject* t)
{
...
static void
checkvalref(global_State* g, GCObject* f, const TValue* t)
{
...
static void
checktable(global_State* g, Table* h)
{
    unsigned int i;
    Node *n, *limit = gnode(h, sizenode(h));
    GCObject* hgc = obj2gco(h);
    checkobjref(g, hgc, h->metatable);
    for (i = 0; i < h->sizearray; i++) checkvalref(g, hgc, &h->array[i]);
...
*/
static void
checkproto(global_State* g, Proto* f)
{
    int i;
    ...
    for (i = 0; i < f->sizek; i++) {
        if (ttisstring(f->k + i)) checkobjref(g, fgc, tsvalue(f->k + i));
    }
    for (i = 0; i < f->sizeupvalues; i++) checkobjref(g, fgc, f->upvalues[i].name);
    for (i = 0; i < f->sizep; i++) checkobjref(g, fgc, f->p[i]);
    for (i = 0; i < f->sizelocvars; i++) checkobjref(g, fgc, f->locvars[i].varname);
}


static void
checkCclosure(global_State* g, CClosure* cl)
{
    GCObject* clgc = obj2gco(cl);
    int i;
    for (i = 0; i < cl->nupvalues; i++) checkvalref(g, clgc, &cl->upvalue[i]);
}


static void
checkLclosure(global_State* g, LClosure* cl)
{
    GCObject* clgc = obj2gco(cl);
    int i;
...
static int
lua_checkpc(lua_State* L, CallInfo* ci)
{
...
static void
checkstack(global_State* g, lua_State* L1)
{
    StkId o;
    CallInfo* ci;
    UpVal* uv;
    lua_assert(!isdead(g, L1));
    for (uv = L1->openupval; uv != NULL; uv = uv->u.open.next) lua_assert(upisopen(uv)); /* must be open */
...
static void
checkobject(global_State* g, GCObject* o, int maybedead)
{
    if (isdead(g, o)) lua_assert(maybedead);
    else {
...
static void
checkgraylist(global_State* g, GCObject* o)
{
...
static void
checkgray(global_State* g, GCObject* o)
{
    for (; o != NULL; o = o->next) {
        if (isgray(o)) {
...
lua_checkmemory(lua_State* L)
{
    ...
    markgrays(g);
    /* check 'fixedgc' list */
    for (o = g->fixedgc; o != NULL; o = o->next) { lua_assert(o->tt == LUA_TSHRSTR && isgray(o)); }
    /* check 'allgc' list */
    checkgray(g, g->allgc);
...
static int
settrick(lua_State* L)
{
    if (ttisnil(obj_at(L, 1))) l_Trick = NULL;
    else l_Trick = gcvalue(obj_at(L, 1));
    return 0;
}


static int
gc_color(lua_State* L)
{
    TValue* o;
    luaL_checkany(L, 1);
    o = obj_at(L, 1);
    if (!iscollectable(o)) lua_pushstring(L, "no collectable");
    else {
...
static int
gc_state(lua_State* L)
{
    static const char* statenames[] = {
        "propagate", "atomic", "sweepallgc", "sweepfinobj", "sweeptobefnz", "sweepend", "pause", ""};
    static const int states[] = {
        GCSpropagate, GCSatomic, GCSswpallgc, GCSswpfinobj, GCSswptobefnz, GCSswpend, GCSpause, -1};
...
table_query(lua_State* L)
{
    ...
    if (i == -1) {
        ...
        else lua_pushliteral(L, "<undef>");
        pushobject(L, gval(gnode(t, i)));
        if (gnext(&t->node[i]) != 0) lua_pushinteger(L, gnext(&t->node[i]));
        else lua_pushnil(L);
    }
    return 3;
}
...
doonnewstack(lua_State* L)
{
    ...
    if (status == LUA_OK) status = lua_pcall(L1, 0, 0, 0);
...
doremote(lua_State* L)
{
    ...
    if (status == LUA_OK) status = lua_pcall(L1, 0, LUA_MULTRET, 0);
    if (status != LUA_OK) {
        lua_pushnil(L);
...
checkpanic(lua_State* L)
{
    ...
    else { /* error handling */ /* move error message to original state */
        lua_pushstring(L, lua_tostring(L1, -1));
    }
    lua_close(L1);
    return 1;
...
static int
getnum_aux(lua_State* L, lua_State* L1, const char** pc)
{
    int res = 0;
    int sig = 1;
    skip(pc);
    ...
    if (!lisdigit(cast_uchar(**pc))) luaL_error(L, "number expected (%s)", *pc);
    while (lisdigit(cast_uchar(**pc))) res = res * 10 + (*(*pc)++) - '0';
    return sig * res;
}

static const char*
getstring_aux(lua_State* L, char* buff, const char** pc)
{
    int i = 0;
    skip(pc);
...
static int
getindex_aux(lua_State* L, lua_State* L1, const char** pc)
{
    skip(pc);
    switch (*(*pc)++) {
    case 'R':
...
static int
runC(lua_State* L, lua_State* L1, const char* pc)
{
    char buff[300];
    int status = 0;
    ...
    for (;;) {
        ...
        if EQ ("") return 0;
            ...
            if (*msg == '\0') msg = NULL; /* to test 'luaL_checkstack' with no message */
            ...
            if (lua_getmetatable(L1, getindex) == 0) lua_pushnil(L1);
        }
        else if EQ ("gettable") {
            ...
            if (L1 != L) {
                int i;
                for (i = 0; i < n; i++) lua_pushstring(L, lua_tostring(L1, -(n - i)));
            }
            return n;
        }
        else if EQ ("rotate") {
...
static int
Cfunck(lua_State* L, int status, lua_KContext ctx)
{
    pushcode(L, status);
...
*/
static void
Chook(lua_State* L, lua_Debug* ar)
{
    const char* scpt;
...
*/
static void
sethookaux(lua_State* L, int mask, int count, const char* scpt)
{
    if (*scpt == '\0') {            /* no script? */
...
sethook(lua_State* L)
{
    if (lua_isnoneornil(L, 1)) lua_sethook(L, NULL, 0, 0); /* turn off hooks */
    else {

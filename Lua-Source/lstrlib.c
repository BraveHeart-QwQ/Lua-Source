...
str_sub(lua_State* L)
{
    ...
    if (start <= end) lua_pushlstring(L, s + start - 1, (size_t)(end - start) + 1);
...
init:                     /* using goto's to optimize tail recursion */
    if (p != ms->p_end) { /* end of pattern? */
        ...
        dflt : {                              /* pattern class plus optional suffix */
            ...
            else { /* matched once */ switch (*ep) { /* handle optional suffix */
                case '?': {                          /* optional */
                    const char* res;
                    if ((res = match(ms, s + 1, ep + 1)) != NULL) s = res;
                    else {
                        p = ep + 1;
...
lmemfind(const char* s1, size_t l1, const char* s2, size_t l2)
{
    if (l2 == 0) return s1;        /* empty strings are everywhere */
        ...
        while (l1 > 0 && (init = (const char*)memchr(s1, *s2, l1)) != NULL) {
            ...
            else { /* correct 'l1' and 's1' to try again */ l1 -= init - s1;
                s1 = init;
            }
        }
...
push_onecapture(MatchState* ms, int i, const char* s, const char* e)
{
    if (i >= ms->level) {
        ...
        else luaL_error(ms->L, "invalid capture index %%%d", i + 1);
    }
    else {
...
static void
prepstate(MatchState* ms, lua_State* L, const char* s, size_t ls, const char* p, size_t lp)
{
    ms->L          = L;
...
add_value(MatchState* ms, luaL_Buffer* b, const char* s, const char* e, int tr)
{
    ...
    else if (!lua_isstring(L, -1)) luaL_error(L, "invalid replacement value (a %s)", luaL_typename(L, -1));
...
str_gsub(lua_State* L)
{
    ...
    while (n < max_s) {
        ...
        else if (src < ms.src_end) /* otherwise, skip one character */ luaL_addchar(&b, *src++);
...
*/
static lua_Number
adddigit(char* buff, int n, lua_Number x)
{
...
static int
num2straux(char* buff, int sz, lua_Number x)
{
    /* if 'inf' or 'NaN', format it like '%g' */
    if (x != x || x == (lua_Number)HUGE_VAL || x == -(lua_Number)HUGE_VAL)
        return l_sprintf(buff, sz, LUA_NUMBER_FMT, (LUAI_UACNUMBER)x);
    else if (x == 0) { /* can be -0... */ /* create "0" or "-0" followed by exponent */
...
static int
lua_number2strx(lua_State* L, char* buff, int sz, const char* fmt, lua_Number x)
{
    ...
    else if (fmt[SIZELENMOD] != 'a') return luaL_error(L, "modifiers for format '%%a'/'%%A' not implemented");
    return n;
}
...
addquoted(luaL_Buffer* b, const char* s, size_t len)
{
    ...
    while (len--) {
        if (*s == '"' || *s == '\\' || *s == '\n') {
            ...
            char buff[10];
            if (!isdigit(uchar(*(s + 1)))) l_sprintf(buff, sizeof(buff), "\\%d", (int)uchar(*s));
            else l_sprintf(buff, sizeof(buff), "\\%03d", (int)uchar(*s));
...
addliteral(lua_State* L, luaL_Buffer* b, int arg)
{
    ...
    case LUA_TNUMBER: {
        ...
        else { /* integers */ lua_Integer n = lua_tointeger(L, arg);
            const char* format =
...
str_format(lua_State* L)
{
    ...
    while (strfrmt < strfrmt_end) {
        if (*strfrmt != L_ESC) luaL_addchar(&b, *strfrmt++);
        ...
        else { /* format item */ char form[MAX_FORMAT];             /* to store the format ('%...') */
            ...
            case 's': {
                ...
                if (form[2] == '\0')   /* no modifiers? */
                    ...
                    else { /* format the string into 'buff' */ nb = l_sprintf(buff, MAX_ITEM, form, s);
...
*/
static KOption
getdetails(Header* h, size_t totalsize, const char** fmt, int* psize, int* ntoalign)
{
...
*/
static void
copywithendian(volatile char* dest, volatile const char* src, int size, int islittle)
{
...
str_pack(lua_State* L)
{
    ...
    while (*fmt != '\0') {
        ...
        case Kstring: { /* strings with length count */
            ...
            const char* s = luaL_checklstring(L, arg, &len);
            luaL_argcheck(
                L, size >= (int)sizeof(size_t) || len < ((size_t)1 << (size * NB)), arg,

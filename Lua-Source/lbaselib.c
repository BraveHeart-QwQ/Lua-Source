...
static const char*
b_str2int(const char* s, int base, lua_Integer* pn)
{
    lua_Unsigned n = 0;
    int neg        = 0;
...
static int
pairsmeta(lua_State* L, const char* method, int iszero, lua_CFunction iter)
{
    luaL_checkany(L, 1);
...
generic_reader(lua_State* L, void* ud, size_t* size)
{
    ...
    else if (!lua_isstring(L, -1)) luaL_error(L, "reader function must return a string");
...
luaB_load(lua_State* L)
{
    ...
    else { /* loading from a reader function */ const char* chunkname = luaL_optstring(L, 2, "=(load)");
...
luaB_assert(lua_State* L)
{
    ...
    else { /* error */ luaL_checkany(L, 1);      /* there must be a condition */
...
finishpcall(lua_State* L, int status, lua_KContext extra)
{
    ...
    else return lua_gettop(L) - (int)extra; /* return all results */
}

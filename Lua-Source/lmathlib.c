...
static int
math_toint(lua_State* L)
{
    int valid;
    lua_Integer n = lua_tointegerx(L, 1, &valid);
    if (valid) lua_pushinteger(L, n);
    else {
        luaL_checkany(L, 1);
...
static void
pushnumint(lua_State* L, lua_Number d)
{
    lua_Integer n;
...
{
    if (lua_isinteger(L, 1)) lua_settop(L, 1); /* integer is its own ceil */
    else {

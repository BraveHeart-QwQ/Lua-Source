...
*/
static int
iscleared(global_State* g, const TValue* o)
{
...
luaC_barrier_(lua_State* L, GCObject* o, GCObject* v)
{
    ...
    else { /* sweep phase */ lua_assert(issweepphase(g));
...
*/
void
luaC_barrierback_(lua_State* L, Table* t)
{
    global_State* g = G(L);
...
*/
void
luaC_upvalbarrier_(lua_State* L, UpVal* uv)
{
    global_State* g = G(L);
...
*/
GCObject*
luaC_newobj(lua_State* L, int tt, size_t sz)
{
    global_State* g = G(L);
...
remarkupvals(global_State* g)
{
    ...
    while ((thread = *p) != NULL) {
        ...
        else { /* thread is not marked or without upvalues */ UpVal* uv;
...
traverseweakvalue(global_State* g, Table* h)
{
    ...
    if (g->gcstate == GCSpropagate) linkgclist(h, g->grayagain); /* must retraverse it in atomic phase */
    else if (hasclears) linkgclist(h, g->weak);                  /* has to be cleared later */
}
...
*/
static int
traverseephemeron(global_State* g, Table* h)
{
    ...
    for (n = gnode(h, 0); n < limit; n++) {
        ...
        else if (valiswhite(gval(n))) { /* value not marked yet? */ marked = 1;
...
traversetable(global_State* g, Table* h)
{
    ...
    if (mode && ttisstring(mode) && /* is there a weak mode? */
        ...
        else if (!weakvalue) /* strong values? */ traverseephemeron(g, h);
        else /* all weak */ linkgclist(h, g->allweak); /* nothing to traverse now */
    }
    else /* not weak */ traversestrongtable(g, h);
...
traverseproto(global_State* g, Proto* f)
{
    ...
    return sizeof(Proto) + sizeof(Instruction) * f->sizecode + sizeof(Proto*) * f->sizep +
           sizeof(TValue) * f->sizek + sizeof(int) * f->sizelineinfo + sizeof(LocVar) * f->sizelocvars +
...
traversethread(global_State* g, lua_State* th)
{
    ...
    else if (g->gckind != KGC_EMERGENCY) luaD_shrinkstack(th); /* do not change stack in emergency cycle */
...
*/
static void
clearkeys(global_State* g, GCObject* l, GCObject* f)
{
    for (; l != f; l = gco2t(l)->gclist) {
        Table* h = gco2t(l);
        Node *n, *limit = gnodelast(h);
        for (n = gnode(h, 0); n < limit; n++) {
            if (!ttisnil(gval(n)) && (iscleared(g, gkey(n)))) { setnilvalue(gval(n)); /* remove value ... */ }
...
*/
static void
clearvalues(global_State* g, GCObject* l, GCObject* f)
{
    for (; l != f; l = gco2t(l)->gclist) {
        Table* h = gco2t(l);
        Node *n, *limit = gnodelast(h);
...
void
luaC_upvdeccount(lua_State* L, UpVal* uv)
{
...
static void
freeLclosure(lua_State* L, LClosure* cl)
{
    int i;
...
sweeplist(lua_State* L, GCObject** p, lu_mem count)
{
    ...
    while (*p != NULL && count-- > 0) {
        ...
        else { /* change mark to 'white' */ curr->marked = cast_byte((marked & maskcolors) | white);
...
static void
GCTM(lua_State* L, int propagateerrors)
{
    global_State* g = G(L);
    const TValue* tm;
    TValue v;
...
luaC_checkfinalizer(lua_State* L, GCObject* o, Table* mt)
{
    ...
    else { /* move 'o' to 'finobj' list */ GCObject** p;
        if (issweepphase(g)) {
...
setpause(global_State* g)
{
    ...
    threshold =
        (g->gcpause < MAX_LMEM / estimate) /* overflow? */
...
singlestep(lua_State* L)
{
    ...
    case GCScallfin: { /* call remaining finalizers */
        ...
        else { /* emergency mode or no more finalizers */ g->gcstate = GCSpause; /* finish collection */
            return 0;
        }
    }
    default:
...
*/
void
luaC_runtilstate(lua_State* L, int statesmask)
{
    global_State* g = G(L);
...
luaC_step(lua_State* L)
{
    ...
    if (g->gcstate == GCSpause) setpause(g); /* pause until next cycle */
    else {
